package units.advanceprogramming.expressionparser;
  ...
  public Node parse() throws IllegalArgumentException {
    ...
    if (token != null && token.start == cursor) {
      ...
      if (closedBracketToken != null && closedBracketToken.start == cursor) {
        ...
        if (token.start == 0 && cursor != string.length() && string.charAt(cursor) != ')') {
          throw new IllegalArgumentException(String.format(
                  "Unexpected string '%s' at the end",
        ...
        if (cursor != string.length()) {
          ...
          throw new IllegalArgumentException("missing bracket at the end");
        }
      }
